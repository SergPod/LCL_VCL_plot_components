...
uses
  ...
  sgr_point, sgr_def, sgr_xydata_series, sgr_mark,
  set_axis_dlg_laz;
type
  ...
  TForm1 = class(TForm)
    ...
    procedure ckbDoOnFieldDrawChange(Sender: TObject);
...
begin
  ...
  if Screen.PixelsPerInch <> DesignTimePPI then
    plt.Rescale(Screen.PixelsPerInch / DesignTimePPI);
end;
...
procedure TForm1.ckbDoOnFieldDrawChange(Sender: TObject);
begin
  if ckbDoOnFieldDraw.Checked then
    plt.OnFieldDraw := XYPlotFieldDraw
  else
